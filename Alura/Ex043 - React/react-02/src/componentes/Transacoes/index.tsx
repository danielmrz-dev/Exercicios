...
const estadoInicial: Omit<ITransacao, "id"> = {
  nome: "",
  valor: 0,
  categoria: "",
  data: "",
...
const Transacoes = () => {
  ...
  const { criaNovaTransacao, transacoes } = useAppContext();

  const [novaTransacao, setNovaTransacao] = useState<Omit<ITransacao, "id">>(estadoInicial);
  ...
  const aoCriarTransacao = async () => {
    try {
      await criaNovaTransacao(novaTransacao);
