...
function mostrarJobs(jobsList) {
    ...
    jobsList.forEach((job) => {
        let newOrNot = job.new == true ? '<span class="job__new">New!</span>' : '';
        let featuredOrNot = job.featured == true ? '<span class="job__featured">Featured</span>' : '';
        let border = job.featured == true ? '.active-border' : '';

        let languagesTags = job.languages.map(language => `<span class="job__tag job__tag-languages">${language}</span>`).join('');
        let toolsTags = job.tools.map(tool => `<span class="job__tag job__tag-tools">${tool}</span>`).join('');
